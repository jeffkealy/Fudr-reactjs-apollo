      ...
      mainQuery: {
          ...
          resolve: (root,{first,after}) => {
                ...
                var edgesAndPageInfoPromise = new Promise((resolve,reject)=>{
                   ...
                   console.log("edgesAndPageInfoPromise");
                   let edges =  Dish.where('id').gt(cursorNumeric).find({},(err,result)=>{
                          if (err){
                            console.error("---Error "+ err)
                          }
                       }).limit(first).cursor() //<<<TIP>>> Limit is obtained from Args.first
                   edges.on('data',res => {
                     console.log("edges.on", res);
                   ...
                   edges.on('end',() => {
                      ...
                      let hasNextPageFlag = new Promise((resolve,reject)=>{
                          if (endCursor){
                            ...
                            console.log("endCursorNumeric", endCursorNumeric);
                            Dish.where('id').gt(endCursorNumeric).count((err,count)=>{
                ...
                let totalCountPromise = new Promise((resolve,reject)=>{
                    if (totalCount ===0) {
                      totalCount = Dish.count((err,count)=>{
                        if (err) reject(err)
                        resolve(count)
                      })
                    }
